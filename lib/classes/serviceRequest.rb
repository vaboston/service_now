module ServiceNow
    class ServiceRequest
        ...
        def initialize(attributes = {}, saved_on_sn = false, internal_call = false)
            puts "Calls"
	    ServiceRequest.check_configuration
        ...
        def self.find(inc_number)
            ...
            query_hash = {}
            query_hash[:number] = "SRQ" + "0"*(7-inc_string.length) + inc_string
            response = Configuration.get_resource(query_hash, table = "u_service_request").get();
            # returned hash
            hash = JSON.parse(response, { :symbolize_names => true })
